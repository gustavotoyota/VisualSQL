...
export default {
  ...
  mounted() {
    ...
    function onLoad() {
      setTimeout(() => {
        ...
        if (localStorage.getItem("showWelcomeDialog") === "false") return;

        setTimeout(() => {
          // Start dialog animation

          $state.help.title = "Welcome to VisualSQL!";
    ...
    document.addEventListener(
      "pointerdown",
      this.onDocumentCapturePointerDown,
      true
    );
  ...
  methods: {
    ...
    onDocumentPointerMove(event) {
      ...
      if (pointers.length >= 2) {
        ...
        if ($state.pinching.centerPos != null) {
          ...
          currentModule.camera.pos.x =
            -centerOffset.x / currentModule.camera.zoom +
            worldPos.x +
            (currentModule.camera.pos.x - worldPos.x) / ratio;
          currentModule.camera.pos.y =
            -centerOffset.y / currentModule.camera.zoom +
            worldPos.y +
            (currentModule.camera.pos.y - worldPos.y) / ratio;

          // Camera zoom update

          currentModule.camera.zoom = Math.min(
            Math.max(currentModule.camera.zoom * ratio, $app.minZoom),
            $app.maxZoom
          );
        }
      ...
      if ($state.panning.active) {
        currentModule.camera.pos.x -=
          (displayPos.x - $state.panning.currentPos.x) /
          currentModule.camera.zoom;
        currentModule.camera.pos.y -=
          (displayPos.y - $state.panning.currentPos.y) /
          currentModule.camera.zoom;
        ...
        if (
          ...
          if (dist > 5) $state.panning.selectTimeout = null;
        }
      ...
      if ($state.dragging.active) {
        for (const nodeId of Object.keys(currentTab.nodes.selected)) {
          ...
          node.pos.x +=
            (displayPos.x - $state.dragging.currentPos.x) /
            currentModule.camera.zoom;
          node.pos.y +=
            (displayPos.y - $state.dragging.currentPos.y) /
            currentModule.camera.zoom;
        }
      ...
      if ($state.linking.active) {
        ...
        else $state.linking.newLink.from = $utils.shallowCopy(worldPos);
      ...
      if ($state.nodeCreation.active && !$state.nodeCreation.visible) {
        const dist = Math.sqrt(
          Math.pow(
            $state.pointer.pagePos.x - $state.nodeCreation.dragStartPos.x,
            2
          ) +
            Math.pow(
              $state.pointer.pagePos.y - $state.nodeCreation.dragStartPos.y,
              2
            )
        );
    ...
    onDocumentPointerUp(event) {
      ...
      if ($state.nodeCreation.active) $state.nodeCreation.active = false;
      ...
      if (event.button === 0 && $state.selecting.active) {
        const worldStart = $getters.screenToWorld(
          currentModule,
          $state.selecting.startPos
        );
        const worldEnd = $getters.screenToWorld(
          currentModule,
          $state.selecting.endPos
        );
        ...
        for (const node of Object.values(currentModule.data.nodes.map)) {
          if (
            node.pos.x < topLeft.x ||
            node.pos.x > bottomRight.x ||
            node.pos.y < topLeft.y ||
            node.pos.y > bottomRight.y
          )
            continue;

          if (node.id in currentTab.nodes.selected)
            $delete(currentTab.nodes.selected, node.id);
          else $set(currentTab.nodes.selected, node.id, true);
        ...
        for (const link of Object.values(currentModule.data.links.map)) {
          const linkPos = {
            x:
              (currentModule.data.nodes.map[link.from].pos.x +
                currentModule.data.nodes.map[link.to].pos.x) /
              2,
            y:
              (currentModule.data.nodes.map[link.from].pos.y +
                currentModule.data.nodes.map[link.to].pos.y) /
              2
          };

          if (
            linkPos.x < topLeft.x ||
            linkPos.x > bottomRight.x ||
            linkPos.y < topLeft.y ||
            linkPos.y > bottomRight.y
          )
            continue;

          if (link.id in currentTab.links.selected)
            $delete(currentTab.links.selected, link.id);
          else $set(currentTab.links.selected, link.id, true);
      ...
      if ($state.dragging.active && event.button === 0) {
        ...
        if ($state.dragging.saveState) $commit("saveState");
      }
    ...
    onResize(event) {
      if (innerWidth < 697 && $state.sidebars.left && $state.sidebars.right)
